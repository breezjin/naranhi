    ...
    try {
      ...
      if (data.setupRequired) {
        return {
          success: false,
          error:
            '데이터베이스 테이블이 초기화되지 않았습니다. 관리자에게 문의하세요.',
          data: [],
          setupRequired: true,
        };
      }
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (!noticesResult.success && !categoriesResult.success) {
        setError(
          `데이터를 불러오는 중 오류가 발생했습니다. 공지사항: ${noticesResult.error}, 카테고리: ${categoriesResult.error}`
        );
        ...
        console.warn(
          '카테고리 데이터를 불러오지 못했지만 기본 카테고리를 사용합니다:',
          categoriesResult.error
        );
      }
    } catch (error: unknown) {
      logError('Critical error in fetchData', error, {
        component: 'NoticesPage',
        action: 'fetchData',
      });
  ...
  const filterNotices = useCallback(() => {
    ...
    if (selectedCategory !== 'all') {
      filtered = filtered.filter(
        (notice) => notice.category.name === selectedCategory
      );
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-foreground">공지사항 관리</h2>
          <p className="mt-1 text-muted-foreground">공지사항을 관리합니다</p>
        </div>
                  ...
                  <div className="flex-1">
                    <div className="mb-2 flex items-center gap-3">
                      ...
                      <Badge
                        variant="outline"
                        style={{
                          backgroundColor: `${notice.category.color}20`,
