  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      try {
        ...
        try {
          const [staffResult, facilityResult, noticeResult, publishedResult] =
          ...
          setStats({
            ...
            totalFacilities:
              (facilityResult.status === 'fulfilled'
                ? facilityResult.value.count
                : 0) || 0,
            totalNotices:
              (noticeResult.status === 'fulfilled'
                ? noticeResult.value.count
                : 0) || 0,
            publishedNotices:
              (publishedResult.status === 'fulfilled'
                ? publishedResult.value.count
                : 0) || 0,
          });
        } catch (statsError) {
      ...
      <div className="space-y-6">
        ...
        <Alert>
          <AlertDescription>
            ...
            실제 사용을 위해서는 Supabase 프로젝트를 설정하고 환경변수를
            업데이트해주세요.
            <br />
            <br />
            <strong>다음 단계:</strong>
            ...
            <br />
            2. 환경변수 설정: NEXT_PUBLIC_SUPABASE_URL,
