...
const DEFAULT_OPTIONS: Required<
  Omit<ImageUploadOptions, 'generateThumbnail'>
> & { generateThumbnail?: boolean } = {
...
export async function uploadImage(
  ...
  if (!opts.allowedTypes.includes(file.type)) {
    throw new Error(
      `지원하지 않는 파일 형식입니다. 지원 형식: ${opts.allowedTypes.join(', ')}`
    );
  }
...
export async function deleteImage(
  ...
  try {
    ...
    throw error instanceof Error
      ? error
      : new Error('이미지 삭제 중 알 수 없는 오류가 발생했습니다.');
  }
}
...
async function getImageDimensions(
  ...
  try {
    ...
    throw new Error(
      `이미지 크기를 읽을 수 없습니다: ${error instanceof Error ? error.message : '알 수 없는 오류'}`
    );
  }
}
